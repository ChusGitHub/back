...
def MFinsertarTicket(request, precio, p): # si p = 1 -> particular, si p = 0 -> grupo
	...
	data = {'error' : 0 ,'numero': num_ticket, 'precio': precio, 'fecha' : datetime.strftime(ticket.fecha, "%d.%m.%y"), 'punto' : ticket.punto_venta.nombre, 'particular' : part_bool}
	lock.release()
	return HttpResponse(json.dumps(data), 'application/json')
...
def MFmodificarTicket(request, numero, precio):
	...
	try:
		...
		return HttpResponse(ajson.dumps(data), 'application/json')
...
def MFborrarTicket(request, numero):
	# Se borra el ticket en cuestion
	try:
		ticket_a_borrar = Ticket.objects.get(numero = numero, fecha__year = datetime.now().year)
		ticket_a_borrar.delete()

		tickets = Ticket.objects.filter(numero__gt = numero, fecha__year = datetime.now().year)
		for ticket in tickets:
			ticket.numero = ticket.numero - 1
			ticket.save()
		
		data = {'error' : 0, 'ticket_borrado' : numero}
		
		# Resto el numero de tickets borrados a control.num_ticket
		control = Control.objects.get()
		control.num_ticket = control.num_ticket - 1
		control.save()

	except Ticket.DoesNotExist:
		data = {'error' : 1, 'ticket_borrado' : 0}

	return HttpResponse(json.dumps(data), 'application/json')

def MFborrarTicket2(request, numero):

	db = MySQLdb.connect(user = 'admin_chus', db='admin_BDDLosBarkitos', passwd='Otisuhc0', host='vps.losbarkitos.com')
	cursor = db.cursor()
	...
	try: # Se cambia el numero del ultimo ticket al numero del ticket eliminado
		db2 = MySQLdb.connect(user = 'admin_chus', db='admin_BDDLosBarkitos', passwd='Otisuhc0', host='vps.losbarkitos.com')
		cursor2 = db2.cursor()
		...
		db3 = MySQLdb.connect(user = 'admin_chus', db='admin_BDDLosBarkitos', passwd='Otisuhc0', host='vps.losbarkitos.com')
		cursor3 = db3.cursor()
