...
#		viajes = Viaje.objects.filter(barca = filtro_tipo, vendedor = filtro_vend)
	...
	i = 1
	for viaje in viajes:
		datos = {'numero': viaje.numero, 'fecha': strftime(viaje.fecha, "%H:%M:%S"), 'tipo':viaje.barca.tipo, 'punto_venta':viaje.punto_venta.nombre, 'nombre_vendedor':viaje.vendedor.nombre, 'precio':viaje.precio}
...
def ultimoNumero(request, precio_ticket):
	...
	#ticket en negro
		...
		try:
			# Existe un ticket con el mismo numero y se puede hacer en negro
			ticket = Viaje.objects.get(fecha__day = datetime.now().day, fecha__month = datetime.now().month, fecha__year = datetime.now().year, precio = precio_ticket)
			
			dicc['error'] = 'no'
                        dicc['numero'] = ticker.numero
                        dicc['negro'] = 'si'

			print ticket.numero
		except Viaje.DoesNotExist:
			# El numero de ticket no existe y se tiene que hacer en blanco
			print "Numero de ticket no existe"
			dicc['error'] = 'no'
                        dicc['numero'] = num
                        dicc['negro'] = 'no'
			print dicc
...
def primeraLlegar(request):
	...
	try:
		if rio:
			dicc["rio"] = datetime.strftime(rio[0].llegada_base, "%H:%M:%S")
		else:
			dicc["rio"] = "---"
	except Exception, e:
		dicc["rio"] = "---"
	try:
		if electrica:
			dicc["electrica"] = datetime.strftime(electrica[0].llegada_base, "%H:%M:%S")
		else:
			dicc["electrica"] = "---"
	except Exception, e:
		dicc["electrica"] = "---"
	try:
		if whaly:
			dicc["whaly"] = datetime.strftime(whaly[0].llegada_base, "%H:%M:%S")
		else:
			dicc["whaly"] = "---"
	except Exception, e:
		dicc["whaly"] = "---"
	try:
		if gold:
			dicc["gold"] = datetime.strftime(gold[0].llegada_base, "%H:%M:%S")
		else:
			dicc["gold"] = "---"
	except Exception, e:
		dicc["gold"] = "---"
